...
class srCertificate extends ActiveRecord
{
    ...
    public static function getCertificateData(array $options = array())
    {
        ...
        $sql = "SELECT ";
        $sql .= (isset($options['count']) && $options['count']) ? 'COUNT(*) AS count ' : 'cert.*, usr.firstname, usr.lastname, cert_type.title AS cert_type, obj_data.title AS crs_title ';
        ...
        if (isset($options['sort']) && count($options['sort']) && (!isset($options['count']) || !$options['count'])) {
            $replaces = array(
        ...
        if (isset($options['limit']) && count($options['limit']) && (!isset($options['count']) || !$options['count'])) {
